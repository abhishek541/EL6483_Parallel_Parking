...
static void button_gpio_init(void)
{
	...
	__HAL_RCC_GPIOE_CLK_ENABLE(); // Enable clock to GPIOE so PE11 works
	...
	__HAL_RCC_GPIOA_CLK_ENABLE(); // Enable clock to GPIOA so PA13 works
...
void button_init(void)
{
	...
	HAL_NVIC_SetPriority(EXTI3_IRQn, 5, 6);
...
{
	HAL_GPIO_EXTI_IRQHandler(GPIO_PIN_13); 	// This function will first clear the IRQ on EXTI13
											// It will then call the HAL_GPIO_EXTI_Callback below
}
